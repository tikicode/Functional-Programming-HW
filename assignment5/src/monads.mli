...
(*
  ...
  In this assignment you will be refactoring a simple stateful OCaml program to use a state monad 
  (a stateful stack in particular).
...
(*
  ...
  We provide a `State_int` module as an example. It is a simple state monad we used in lecture: the entire state
  was just one integer.
  It has been changed slightly here to remove the unit argument to `get` and `inc` because it is already frozen 
  ...
  The `State_int` module is only an example to help you implement your stack monad later. Your stack monad will 
  likely have a very similar
...
(*
  ...
  We will use a list and the "cons" operation as our stack, so you might imagine an internal stack module to 
  help.
*)
