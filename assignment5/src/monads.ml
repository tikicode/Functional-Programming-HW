...
module Stack_monad :
  ...
  struct
    ...
    let push (x : 'e) : (unit,'e) t =
      fun (s : 'e Stack.t) -> ((), Stack.push s x)
    let pop : ('e, 'e) t =
      fun (s : 'e Stack.t) -> Stack.pop s
    let is_empty : (bool,'e) t =
      fun (s : 'e Stack.t) -> (Stack.is_empty s, s)
  end
...
module Exception_stack :
  ...
  module Stack =
    ...
    module T : Monad.Basic2 with type ('a, 'e) t = 'e Stack.t -> 'a option * 'e Stack.t =
    struct
...
let are_balanced_more_monadic (s : string) : bool =
  match run (main_more_monadic s) with
  | Some result -> result
  | None -> false