          ...
          Branch
            {
              ...
              right =
                Branch
                  { item = { key = "b"; value = 3 }; left = Leaf; right = Leaf };
            };
        right =
          Branch { item = { key = "e"; value = 2 }; left = Leaf; right = Leaf };
      })
    ...
    Branch
      {
        ...
        left =
          Branch
            { item = { key = "a"; value = "a" }; left = Leaf; right = Leaf };
        right =
          Branch
            { item = { key = "e"; value = "e" }; left = Leaf; right = Leaf };
      })
...
let m2 =
  T.(Branch { item = { key = "e"; value = 2 }; left = Leaf; right = Leaf })

let m3 =
  T.(Branch { item = { key = "a"; value = 2 }; left = Leaf; right = Leaf })
...
let test_of_list_multi _ =
  ...
  assert_equal
    T.(
      Branch
        {
          item = { key = "d"; value = [ 10; 1; 2 ] };
          left = Leaf;
          right = Leaf;
        })
  @@ D.of_list_multi [ ("d", 10); ("d", 1); ("d", 2) ]
...
let test_merge_with _ =
  ...
  assert_equal
    [ ("1", 0); ("8", 0); ("9", 0) ]
    D.(merge_with ~merger:merge_fun example_dict1 T.Leaf |> to_list);
  assert_equal
    [ ("1", 1); ("8", 1); ("99", 1) ]
    D.(merge_with ~merger:merge_fun T.Leaf example_dict2 |> to_list);
  assert_equal
    [ ("1", 10); ("8", 39); ("9", 0); ("99", 1) ]
...
let test_filter_comments _ =
  ...
  assert_equal "abcdef"
  @@ U.filter_comments "abc(* comment1 *)def(* comment2 *)";
  assert_equal "let (+*) = (+)" @@ U.filter_comments "let (+*) = (+)";
  assert_equal "let (+*) = (+) let (+*) = (+)"
  @@ U.filter_comments
       "let (+*) = (+)(* comment1 *)(* comment2 *) let (+*) = (+)"

let test_filter_non_characters _ =
  assert_equal boof @@ U.filter_non_characters "";
  assert_equal " if x_i   0 then true else false "
  @@ U.filter_non_characters "(if x_i = 0 then true else false)"

let kw1 =
  T.(
    Branch { item = { key = "boof"; value = 4 }; left = T.Leaf; right = T.Leaf })

let test_count_kw_occurance _ =
  assert_equal kw1
  @@ U.count_kw_occurance ~kw_list:[ "boof" ] ~path:"./tests.ml"

let kw2 =
  T.(
    Branch
      {
        item = { key = "test2"; value = 1 };
        left =
          T.Branch
            {
              item = { key = "test1"; value = 2 };
              left = T.Leaf;
              right = T.Leaf;
            };
        right =
...
let test_sort_kw_list_by_value _ =
  ...
  assert_equal [ ("test1", 2); ("test2", 2); ("test3", 1) ]
  @@ U.sort_kw_list_by_value kw3

let test_list_to_sexp _ =
  assert_equal "()" @@ U.list_to_sexp [];
  assert_equal "(((keyword let)(count 5)))" @@ U.list_to_sexp [ ("let", 5) ];
  assert_equal "(((keyword let)(count 5))((keyword and)(count 1)))"
  @@ U.list_to_sexp [ ("let", 5); ("and", 1) ]
...
let series = "Assignment2 Tests" >::: [ tree_tests; dict_tests; utils_tests ]
