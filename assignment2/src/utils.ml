...
let traverse_directory (path : string) : string list =
  let rec traverse_helper (path : string) : string list =
    ...
    in 
    let is_subdirectory dir_item = Sys.is_directory (Filename.concat path dir_item) in
    let ocaml_files = 
      dir_contents 
      |> List.filter (fun file -> is_ml_mli file)
      |> List.map (Filename.concat path) in
    let sub_dir_files = 
      dir_contents
      |> List.filter (fun dir_item -> is_subdirectory dir_item)
      |> List.map (Filename.concat path)
      |> List.map traverse_helper
      |> List.concat in 
...
let read_file_as_string (path : string) : string = Stdio.In_channel.read_all path

let filter_strings (contents : string) : string =
  let rec filter_helper (chars : char list) (inside_string : bool) (acc : char list) =
    match chars with
    | [] -> List.rev acc
    | '"' :: tl when not inside_string ->
      filter_helper tl true acc
    | '"' :: tl when inside_string ->
      filter_helper tl false acc
    | _ :: tl when inside_string ->
      filter_helper tl inside_string acc
    | c :: tl ->
      filter_helper tl inside_string (c :: acc)
  in
...
let filter_comments (contents : string) : string = 
  let rec filter_helper (chars : char list) (acc : char list) (depth : int) (open_seen : bool) =
    match chars with
    | [] -> List.rev acc
    | '(' :: '*' :: tl ->
      filter_helper tl acc (depth + 1) true
    ...
    | c :: tl ->
      filter_helper tl (c :: acc) depth false
    in
    let char_list = String.to_seq contents |> List.of_seq in
...
let clean_file (path : string) : string =
  filter_non_characters (filter_comments (filter_strings (read_file_as_string path)))

let count_kw_occurance ~(kw_list : string list) ~(path: string) : int Simpledict.t = 
  let word_list = String.split_on_char ' ' (clean_file path) in
  let count_kw (occurances : int Simpledict.t) (kw : string) : int Simpledict.t =
    let count = List.fold_left (fun count word ->  if word = kw then count + 1 else count) 0 word_list in 
...
let list_to_sexp (kw_counts : (string * int) list) : string =
  kw_counts
  |> List.map (fun (key, value) -> { keyword = key; count = value } |> sexp_of_z)
  |> Sexplib.Sexp.List
